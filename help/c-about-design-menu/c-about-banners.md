...
El segundo método consiste en utilizar Adobe Dynamic Media Classic, un servicio de administración y publicación de medios dinámicos. Una cuenta válida de Dynamic Media Classic de Adobe le permite administrar y distribuir el contenido de los letreros directamente en Destinatario, Search&amp;Promote, con Dynamic Media Classic. En la búsqueda y comercialización del sitio, se configura el acceso a su cuenta de Dynamic Media Classic. A continuación, abra el navegador de medios de Dynamic Media Classic y seleccione un recurso de medios dinámicos que desee que sirva como pancarta.
...
Puede utilizar los tipos de recursos de Dynamic Media Classic como pancartas en la búsqueda o comercialización del sitio, incluidas las imágenes, los letreros dinámicos y las plantillas, como las plantillas de imágenes o las plantillas de Flash.
...
Consulte [Funciones básicas de plantilla](https://help.adobe.com/en_US/scene7/using/WS60B68844-9054-4099-BF69-3DC998A04D3C.html) en la Guía del usuario de Dynamic Media Classic (Scene7).
...
Puede utilizar [!DNL Banners] para administrar las publicidades de titular y dónde se colocan en su sitio Web. Al agregar un letrero, se hace referencia externamente a la imagen mediante fragmentos de código HTML que se muestran en el momento de la búsqueda.
...
Si tiene una cuenta válida de Adobe Dynamic Media Classic, puede agregar publicidades de titular a través de Scene7 Publishing System.
   ...
   <table> 
    ...
    <tbody> 
      ...
      <td colname="col2"> <p> Permite pegar el código HTML asociado a la pancarta. </p> <p>Se acepta cualquier código HTML, incluido el código CSS rodeado por 
        <code>
          &lt;style&gt; 
        </code> o código JavaScript rodeado por 
        <code>
          &lt;script&gt; 
        </code>. Por ejemplo, el siguiente bloque de código es para una pancarta de texto de tipo Horizontal top: <code> &lt;div&nbsp;style="width:&nbsp;684px;&nbsp;background-image:&nbsp;url('https://www.brough.com/blackb.gif');&nbsp; 
...
## Edición de un letrero {#task_D4081083BE7B40F5A003D1A2F1435AEA}

Utilice [!DNL Edit Banner] para cambiar el nombre del letrero, el HTML del letrero, el tipo de pancarta y las etiquetas asociadas.
...
Si agregó un letrero mediante la búsqueda o comercialización del sitio, también lo edita con Adobe Dynamic Media Classic.
...
1. En la página [!DNL Edit Banner], establezca las opciones que desee.
...
1. En el cuadro de diálogo [!DNL Pick an Asset], en el panel izquierdo, utilice las opciones de navegación de la interfaz de usuario para localizar la carpeta que contiene el recurso digital que desea utilizar para un letrero.
   ...
   >[!NOTE]
   >
   >Las opciones de navegación de recursos no aparecen cuando se edita la pancarta en el cuadro de diálogo [!DNL Change Parameters].
   ...
   **Alternar las opciones de visibilidad de capa**
   ...
   (Opcional) Si el recurso digital que desea utilizar para un letrero no está disponible en la carpeta seleccionada, es posible que tenga que cargarlo. Haga clic en **[!UICONTROL Upload]** y seleccione el archivo y las opciones que desee. El archivo se carga en la carpeta seleccionada.
   ...
   **Opciones avanzadas**
   ...
   Los archivos PSD (archivos Documento de Photoshop) se utilizan con mayor frecuencia en Dynamic Media Classic para crear plantillas. Al cargar un archivo PSD, puede crear una plantilla de Dynamic Media Classic automáticamente a partir del archivo (seleccione la opción **[!UICONTROL Create Template]**).
   ...
   <table> 
    ...
    <tbody> 
      ...
      <td colname="col2"> <p>Las capas del archivo PSD se cargan como imágenes independientes. Puede seleccionar entre las siguientes opciones para decidir cómo desea asignar un nombre a estas imágenes en Scene7 Publishing System: </p> 
        ...
        <li id="li_EB4173B884FC41328CFBDE27DA6D43AA"> <span class="uicontrol"> Usar nombre de archivo PSD y número de anexo  </span> <p>Asigna a las imágenes los nombres de sus capas en el archivo PSD, sin tener en cuenta los nombres de las capas originales. Las imágenes reciben el nombre del archivo de Photoshop y un número de capa anexado. Por ejemplo, la segunda capa de un archivo llamado <span class="codeph"> Anuncio de primavera.psd </span> se denomina <span class="codeph"> Anuncio de primavera_2 </span> aunque tenga un nombre no predeterminado en Photoshop. </p> </li> 
        <li id="li_10B2D2DE2FD24BD08DB56D1D95ABA53D"> <span class="uicontrol"> Usar nombre de archivo PSD y nombre o número de capa  </span> <p>Nombra las imágenes después del archivo PSD seguido del nombre o el número de capa. El número de capa se utiliza si los nombres de capa del archivo PSD son nombres de capa predeterminados de Photoshop. Por ejemplo, una capa con el nombre <span class="codeph"> Etiqueta de precio </span> en un archivo PSD con el nombre <span class="codeph"> Publicidad de primavera </span> se denomina <span class="codeph"> Etiqueta de precio de publicidad de primavera </span>. Una capa con el nombre predeterminado <span class="codeph"> Capa 2 </span> se denomina <span class="codeph"> Anuncio de primavera_2 </span>. </p> </li> 
      ...
      <td colname="col02"> <p>Procesando </p> </td> 
      <td colname="col2"> <p> <span class="uicontrol"> La  </span> opción Rasterizar extrae las páginas del archivo PDF y convierte los gráficos vectoriales en imágenes de mapa de bits.  
...
1. (Opcional) En la ventana emergente [!DNL Pick An Asset], en la lista desplegable [!DNL Actions], realice una de las siguientes acciones:
   ...
   * Haga clic **[!UICONTROL Move]**. En el cuadro de diálogo [!DNL Select a folder to move to], seleccione la carpeta en la que desea mover el recurso digital. Haga clic **[!UICONTROL Move]**.
...
Utilice [!DNL Edit Banner] para cambiar las propiedades y los parámetros de un letrero que ha agregado con Adobe Dynamic Media Classic.
...
Consulte también [Edición de un letrero](../c-about-design-menu/c-about-banners.md#task_D4081083BE7B40F5A003D1A2F1435AEA).

**Para editar un letrero con Adobe Dynamic Media Classic**

1. En el menú del producto, haga clic en **[!UICONTROL Design]** > **[!UICONTROL Banners]**.
1. En la página [!DNL Banners], haga clic ![](assets/icon_edit_16.gif) sobre una miniatura de pancarta que tenga un icono S7 en la esquina inferior izquierda de la ventana de la pancarta.
1. En la página [!DNL Change Parameter], establezca las opciones que desee.
1. Cuando haya terminado de editar la pancarta, haga clic en **[!UICONTROL Save]**.
1. (Opcional) Realice una de las siguientes acciones:
...
1. (Opcional) Realice una o varias de las acciones siguientes:
   ...
   * En la página [!DNL Banners], seleccione el tipo de pancarta que desee encontrar en la lista desplegable **[!UICONTROL Find banner of type]**. Si lo desea, especifique un nombre de etiqueta en el campo de texto **[!UICONTROL with tag]** o un nombre de tipo de pancarta en el campo de texto **[!UICONTROL with name]**. Haga clic **[!UICONTROL Find.]**

   * En la lista desplegable **[!UICONTROL Sort]**, seleccione cómo desea ordenar la lista de los letreros.
   * En la lista desplegable **[!UICONTROL Show]**, seleccione el número de pancartas que desea cargar en la página actual que está viendo.
...
1. Realice uno de los siguientes pasos:
   ...
   * En la barra superior de la página [!DNL Banners], marque **[!UICONTROL Select all]** para seleccionar todos los letreros cargados en la página que se muestra actualmente.
...
## Vista previa de letreros {#task_6AB1F81A984A4DC2ACACD1FE030545E2}

Puede examinar los letreros que ha agregado a la página [!DNL Banners] para vista de su tamaño completo. No se muestra ninguna CSS de la plantilla que afecte a la pancarta.
...
1. En la página [!DNL Banners], haga clic en una miniatura de pancarta para vista de su tamaño completo.
1. Realice uno de los siguientes pasos:
   ...
   * Haga clic en el botón Cerrar para cerrar el cuadro de diálogo de previsualización de letreros y volver a la página [!DNL Banners].
...
O bien, si lo prefiere, puede insertar en directo todos los cambios en cualquier pancarta mediante la opción **[!UICONTROL Push Live]** cerca de la parte inferior de la página [!DNL Banners].
...
1. Realice uno de los siguientes pasos:
   ...
   * En la esquina superior izquierda de cualquier cuadro de pancarta, haga clic en la casilla de verificación de cada pancarta que desee eliminar.
   * En la barra superior de la página [!DNL Banner], marque **[!UICONTROL Select all]** para seleccionar todos los letreros cargados en la página que se muestra actualmente.
